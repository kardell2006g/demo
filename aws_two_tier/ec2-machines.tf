resource "aws_instance" "phpapp" {
  ...
  echo "\$conn = new mysqli('ip-172-16-3-68.us-west-2.compute.internal', 'root', 'secret', 'test');" >> /var/www/html/myApp.php
...
resource "aws_instance" "database" {
  ...
  mysql -u root -psecret -e "INSERT INTO Employees (NAME, ADDRESS) values ('Adam ', 'Detroit') ;" test
HEREDOC
}
